...
class PatientBillableItemInline(admin.TabularInline):
    """Allows viewing individual station charges directly inside the invoice layout view."""
    model = PatientBillableItem
    extra = 0
    fields = ['station', 'service', 'drug', 'name', 'unit_price', 'quantity', 'total_cost', 'is_paid']
    readonly_fields = ['total_cost', 'incurred_at']
...
class PatientInvoiceAdmin(admin.ModelAdmin):
    ...
    """
    list_display = ['id', 'get_queue_id', 'get_patient_name', 'status', 'get_total_payable', 'created_at']
    list_filter = ['status', 'created_at']
    search_fields = ['patient__first_name', 'patient__last_name', 'visit__queue_id', 'visit__health_record_number']
...
class PatientBillableItemAdmin(admin.ModelAdmin):
    ...
    """
    list_display = ['id', 'invoice', 'station', 'name', 'unit_price', 'quantity', 'get_total_cost', 'is_paid', 'incurred_at']
    list_filter = ['station', 'is_paid', 'incurred_at']
    list_editable = ['is_paid']  # Allows quick settlement check toggles direct from list overview grid
    search_fields = ['name', 'invoice__patient__first_name', 'invoice__patient__last_name']
    
    # Safely look up parent keys without heavy memory usage
