...
class PatientBillableItemInline(admin.TabularInline):
    """
    Allows viewing and auditing individual station charges 
    directly inside the master invoice form block.
    """
    model = PatientBillableItem
    extra = 0
    
    # Polymorphic lookup handling using cleaner pop-up search selectors
    raw_id_fields = ['service', 'drug']
    fields = ['station', 'service', 'drug', 'name', 'unit_price', 'quantity', 'total_cost', 'is_paid']
    
    # Custom property methods MUST be defined as read-only here
    readonly_fields = ['total_cost']


# 2. REGISTER THE PARENT MODEL ADMIN (WHICH USES THE INLINE)
@admin.register(PatientInvoice)
class PatientInvoiceAdmin(admin.ModelAdmin):
    """
    Admin control board managing overall billing profiles for outpatient encounters.
    """
    # UPDATED: Added 'invoice_number' to the list display for immediate scannability
    list_display = [
        'id', 
        'invoice_number', 
        'get_queue_id', 
    ...
    # UPDATED: Added 'invoice_number' directly to search fields so admins can find it by typing the code
    search_fields = [
        'invoice_number',
        'patient__name', 
        'visit__queue_id', 
    ...
    inlines = [PatientBillableItemInline]
    ordering = ['-created_at']
    
    # PERFORMANCE HOOK: Eliminates N+1 query loops on custom related display lists
    list_select_related = ['visit', 'patient']
    ...
    def get_queue_id(self, obj):
        ...
        return "N/A"
    get_queue_id.short_description = 'Queue Token'
    get_queue_id.admin_order_field = 'visit__queue_id'

    def get_total_payable(self, obj):
        return f"KES {obj.total_payable:,.2f}"
    get_total_payable.short_description = 'Total Balance Due'

    def get_status_badge(self, obj):
        """Renders clear, colorful UI badges for invoices based on status choice entries."""
        colors = {
            'PAID': '#10b981',               # Vibrant Green
            'PARTIAL': '#f59e0b',            # Amber Orange
            'UNPAID': '#ef4444',             # Coral Red
            'INSURANCE_PENDING': '#3b82f6',  # Royal Blue
        }
        ...
        return format_html(
            ...
            'font-weight: 700; font-size: 10px; border-radius: 12px; '
            'text-transform: uppercase; font-family: monospace;">{}</span>',
            color, status_label
        )
    get_status_badge.short_description = 'Billing Status'
    get_status_badge.admin_order_field = 'status'


# 3. REGISTER THE STANDALONE LINE-ITEM LEDGER ADMIN
@admin.register(PatientBillableItem)
class PatientBillableItemAdmin(admin.ModelAdmin):
    """
    Admin control board supervising individual ledger lines generated by charging stations.
    """
    list_display = [
        'id', 'get_invoice_link', 'station', 'name', 
        'unit_price', 'quantity', 'get_total_cost', 'is_paid', 'incurred_at'
    ]
    list_filter = ['station', 'is_paid', 'incurred_at']
    
    # Allows the cashier to toggle individual item payments directly from the list grid
    list_editable = ['is_paid'] 
    
    # UPDATED: Added 'invoice__invoice_number' to line-item search parameters
    search_fields = ['name', 'invoice__invoice_number', 'invoice__patient__name', 'invoice__visit__queue_id']
    
    # Memory optimization configuration for long financial database tables
    raw_id_fields = ['invoice', 'service', 'drug']
    ordering = ['-incurred_at']
    
    # PERFORMANCE HOOK: Prevents backend locking by pre-fetching relations across linked records
    ...
    def get_invoice_link(self, obj):
        """Generates a clickable structural link direct to the parent invoice form displaying its code."""
        url = reverse("admin:core_patientinvoice_change", args=[obj.invoice.id])
        # UPDATED: Dynamically presents invoice_number format text string if available, falls back to ID
        display_name = obj.invoice.invoice_number or f"#{obj.invoice.id}"
        return format_html(
            '<a href="{}" style="font-weight: bold; color: #0284c7;">{} &rarr;</a>', 
            url, display_name
        )
    get_invoice_link.short_description = 'Parent Ledger Context'
    get_invoice_link.admin_order_field = 'invoice__invoice_number'
